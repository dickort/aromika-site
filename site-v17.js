  ...
  function initARPQ(){
    ...
    if('IntersectionObserver' in window){
      var io=new IntersectionObserver(function(entries){
        entries.forEach(function(entry){
          if(entry.isIntersecting){
            var factory=root.querySelector('.arpq-factory');
            if(factory){
              factory.animate(
                [{opacity:.84,transform:'translateY(20px)'},{opacity:1,transform:'translateY(0)'}],
            ...
            root.querySelectorAll('.arpq-step').forEach(function(el,i){
              el.animate(
                ...
                {duration:650,delay:90+i*95,easing:'cubic-bezier(.22,1,.36,1)',fill:'both'}
              );
            });
            io.disconnect();
          }
...
(function(){
 function initArg(){
  ...
  function loadRegions(){
   ...
   regionPromise=fetch('https://cdn.jsdelivr.net/gh/galymorg/new_qazaqstan_GeoJSON@main/regions.json')
    .then(function(r){if(!r.ok)throw new Error('regions '+r.status);return r.json()})
    .then(function(d){regionData=d;return d})
