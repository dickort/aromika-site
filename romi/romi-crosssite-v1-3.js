/* AROMIKA · ROMI CROSS-SITE BRIDGE V1.3.0
 * Keeps one ROMI session/context between aromika.info and aromika.shop.
 * Customer identity remains server-side in CS-Cart; browser carries only session_id/context.
 */
(function(){
'use strict';
if(window.__AROMIKA_ROMI_CROSSSITE_V13__)return;
window.__AROMIKA_ROMI_CROSSSITE_V13__=true;
...
var API_SAVE='https://aromika.shop/index.php?dispatch=romi.session_save';
var API_CUSTOMER='https://aromika.shop/index.php?dispatch=romi.customer_status';
...
function saveRemote(){
 ...
 try{
  return fetch(API_SAVE,{method:'POST',mode:'cors',credentials:'include',keepalive:true,headers:{'Accept':'application/json','Content-Type':'application/json'},body:JSON.stringify({session_id:sid,session:session})})
...
function syncCustomerStatus(){
 var sid=getSid();
 try{
  return fetch(API_CUSTOMER,{method:'POST',mode:'cors',credentials:'include',headers:{'Accept':'application/json','Content-Type':'application/json'},body:JSON.stringify({session_id:sid})})
   .then(function(r){return r.json().catch(function(){return null})})
   .then(function(j){
    if(j&&j.customer){
     window.__ROMI_CUSTOMER_BRIDGE_STATUS__=j.customer;
     try{window.dispatchEvent(new CustomEvent('romi:customer-status',{detail:j.customer}))}catch(e){}
    }
    return j;
...
window.addEventListener('pagehide',function(){saveRemote();});
window.addEventListener('focus',function(){syncCustomerStatus();});
window.AROMIKA_ROMI_BRIDGE={decorate:decorate,save:saveRemote,context:collectContext,sessionId:getSid,customerStatus:syncCustomerStatus};
})();
