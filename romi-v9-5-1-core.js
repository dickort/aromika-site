(function(){
'use strict';
if (window.__ROMI_V951__) return;
window.__ROMI_V951__ = true;
...
var LOCAL_STATE_KEY = 'romi-v8-local-state-v2';
...
function sessionPayload(){
  return {
    ...
    last_products:Array.isArray(STATE.last_products)?STATE.last_products.slice(0,8):[],
    customer_type:STATE.customer_type||'',
    city:STATE.city||'',
    selected_product_id:Number(STATE.selected_product_id)||0,
    lang:uiLang(),
    updated_at:Date.now()
  };
}
...
function loadLocalSession(){
  try{
    var j=JSON.parse(localStorage.getItem(LOCAL_STATE_KEY)||'null');
    if(j&&j.session_id===STATE.session_id&&j.session&&typeof j.session==='object')return j.session;
  }catch(e){}
  return null;
}
function saveSession(){
  var payload=sessionPayload();
  saveLocalSession(payload); // synchronous: survives immediate page navigation
...
function loadSession(){
  ...
  if(local)applySessionData(local);

  // Then merge server state if it is newer.
  return api('session_get',{query:{session_id:STATE.session_id}}).then(function(j){
    STATE.session_id=j.session_id||STATE.session_id;
    var remote=j.session||{};
    var localTs=Number(local&&local.updated_at)||0;
    var remoteTs=Number(remote&&remote.updated_at)||0;
    if(remoteTs>localTs)applySessionData(remote);
    try{localStorage.setItem(SID_KEY,STATE.session_id)}catch(e){}
    saveLocalSession(remoteTs>localTs?remote:(local||sessionPayload()));
  }).catch(function(){
    // Local state is enough for same-browser page-to-page continuity.
    saveLocalSession(local||sessionPayload());
  });
}
function persistBeforeLeave(){
  ...
  try{
    if(navigator.sendBeacon){
      var url=C.apiBase+encodeURIComponent('session_save');
      var blob=new Blob([JSON.stringify({session_id:STATE.session_id,session:payload})],{type:'application/json'});
...
function build(){
  ...
  loadSession().then(function(){
    ...
    return restoreLastProducts().then(function(){
      var shouldOpen=false;
      try{shouldOpen=sessionStorage.getItem(COMMERCE_OPEN_KEY)==='1'}catch(e){}
      if(shouldOpen)open();
...
function open(){
  applyUiLanguage();
  try{sessionStorage.setItem(COMMERCE_OPEN_KEY,'1')}catch(e){}
...
function closeToHome(){
  try{sessionStorage.setItem(COMMERCE_OPEN_KEY,'0')}catch(e){}
...
function restoreLastProducts(){
  var ids=Array.isArray(STATE.last_products)?STATE.last_products.slice(0,6):[];
  if(!ids.length)return Promise.resolve();
  return Promise.all(ids.map(function(id){
    return api('product',{query:{product_id:id}})
      .then(function(j){return j.product||null})
      .catch(function(){return null});
  })).then(function(items){
    var products=items.filter(Boolean);
...
function boot(){
  window.addEventListener('romi:language',function(){applyUiLanguage();saveSession()});
  window.addEventListener('pagehide',persistBeforeLeave);
  document.addEventListener('visibilitychange',function(){if(document.visibilityState==='hidden')persistBeforeLeave()});
  fixLegacyInfoLinks();
