(function(){
'use strict';
if(window.__AROMIKA_ASSISTANT_V5_PRODUCTION__) return;
window.__AROMIKA_ASSISTANT_V5_PRODUCTION__=true;
...
function context(){var p=path(),h=location.hash||'';if(['/perfect','/washexpert','/maxipower','/prachka','/antibak','/brands'].indexOf(p)>=0)return'brand';if(p==='/akcii')return'promo';if(p==='/okompanii'&&h==='#production-quality')return'production';if(p==='/okompanii')return'company';if(p==='/kontakty')return'contacts';if(p==='/careers')return'vacancy';return'home'}
...
function hrefFor(k){if(k==='shop')return'https://aromika.shop/';if(k==='sale')return'/akcii';if(k==='brands')return'/brands';if(k==='contacts')return'/kontakty';if(k==='work')return'/careers';if(k==='production')return'/okompanii#production-quality';return''}
...
function init(){var old=document.getElementById('aromika-assistant');if(old)old.remove();preload();var root=shell();render(root);bindRoot(root);scheduleTeaser(root)}
