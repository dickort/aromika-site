...
if(window.__AROMIKA_ASSISTANT_V952__) return;
...
function scriptBase(){
  ...
  if(!src){var list=document.getElementsByTagName('script');for(var i=list.length-1;i>=0;i--){if((list[i].src||'').indexOf('assistant-v9-5-2.js')>=0){src=list[i].src;break}}}
...
function hasChosenRomiLanguage(){try{return !!(localStorage.getItem(ROMI_LANG_KEY)||localStorage.getItem(LANG_KEY))}catch(e){return false}}
function loadState(){
 var saved=null;
 ...
 var allowed=['welcome','home','finder','finderNeeds','finderVolumes','finderResult','repRequest','rep','partner','partnerContact','feedback','feedbackSuccess'];
 if(allowed.indexOf(next.screen)<0)next.screen='home';
 if(hasChosenRomiLanguage()&&next.screen==='welcome')next.screen='home';
...
function saveState(){
 try{localStorage.setItem(UI_STATE_KEY,JSON.stringify({
  ...
  finderCategory:state.finderCategory||'',finderNeed:state.finderNeed||'',finderVolume:state.finderVolume||'',
  repPurpose:state.repPurpose||'',repCity:state.repCity||'',coopType:state.coopType||'',
