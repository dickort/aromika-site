...
function build(){
  ...
  LAYER.innerHTML=
    ...
    '<div class="rv81-quick">'+
      ...
      '<button type="button" data-q="чистящее средство">Уборка</button>'+
      '<button type="button" data-q="жидкое мыло">Мыло</button>'+
      '<button type="button" data-q="гель для душа">Душ и тело</button>'+
    '</div>'+
...
function clicks(e){
  ...
  if(brand){
    ...
    if(!brandName) return;
    CHAT.innerHTML='';
    say('Показываю товары '+brandName+'.');
...
function renderBrandPicker(){
  ...
  addMessage('assistant',
    '<div class="rv91-brand-picker">'+
      '<div class="rv91-brand-picker-head"><b>Наши бренды</b><span>Выберите бренд — покажу товары магазина</span></div>'+
      '<div class="rv91-brand-grid">'+
        ['Perfect','Wash Expert','Maxi Power','Prachka','Antibak'].map(function(name){
          return '<button type="button" data-brand-query="'+esc(name)+'"><b>'+esc(name)+'</b><span>Показать товары →</span></button>';
        }).join('')+
      '</div>'+
    '</div>'
  );
...
function renderProducts(products){
  addMessage('assistant',
    '<div class="rv81-result-head"><b>'+RU.found+'</b><span>'+products.length+' шт.</span></div>'+
...
function fixLegacyInfoLinks(){
  ...
  document.addEventListener('click',function(e){
    ...
    if(commerceBrandRoutes[pathOnly]){
      ...
      say('Показываю товары '+brandQuery+'.');
    ...
    if(raw.indexOf('/#products')===0){
      ...
      open();
      CHAT.innerHTML='';
      say('Показываю продукцию Aromika.');
      smartSearch('Aromika');
      return;
    }
