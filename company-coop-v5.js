(function(){
 ...
 var dict={
  ru:{
   ...
   coopKicker:'Сотрудничество',coopTitle:'Открыты к новым<br><em>деловым связям.</em>',coopLead:'Выберите направление: закупки и HoReCa ведут региональные команды, а дистрибуционные и производственные предложения рассматривает центральный офис.',
   businessTag:'Коммерческим партнёрам',businessTitle:'Стать партнёром Aromika',businessText:'Выберите подходящее направление — ROMI покажет контакт региональной команды или подготовит обращение в центральный офис.',businessRetail:'Закупка продукции Aromika',businessRetailText:'Для торговых сетей, оптовых компаний и розничных клиентов.',businessHoreca:'HoReCa и корпоративные клиенты',businessHorecaText:'Подбор и поставки продукции для регулярных профессиональных задач.',businessDistribution:'Дистрибуция вашего бренда',businessDistributionText:'Для производителей и владельцев торговых марок, которым нужна региональная система продаж.',businessCta:'Продолжить в ROMI',
  ...
  kk:{
   ...
   coopKicker:'Ынтымақтастық',coopTitle:'Жаңа іскерлік<br><em>байланыстарға ашықпыз.</em>',coopLead:'Бағытты таңдаңыз: сатып алу мен HoReCa жұмысын өңірлік командалар жүргізеді, ал дистрибуциялық және өндірістік ұсыныстарды орталық кеңсе қарайды.',
   businessTag:'Коммерциялық серіктестерге',businessTitle:'Aromika серіктесі болу',businessText:'Қолайлы бағытты таңдаңыз — ROMI өңірлік команданың байланысын көрсетеді немесе орталық кеңсеге өтініш дайындайды.',businessRetail:'Aromika өнімдерін сатып алу',businessRetailText:'Сауда желілері, көтерме компаниялар және бөлшек клиенттер үшін.',businessHoreca:'HoReCa және корпоративтік клиенттер',businessHorecaText:'Тұрақты кәсіби міндеттерге өнім таңдау және жеткізу.',businessDistribution:'Брендіңізді дистрибуциялау',businessDistributionText:'Өңірлік сату жүйесі қажет өндірушілер мен сауда маркасы иелеріне.',businessCta:'ROMI-де жалғастыру',
 ...
 function highlightRequestedDirection(){var type='';try{type=(new URLSearchParams(location.search).get('type')||'').toLowerCase()}catch(e){}if(type==='distributor')type='distribution';var target=type==='supplier'?page.querySelector('.cc-collab-card--supply'):page.querySelector('[data-business-type="'+type+'"]');if(target)target.classList.add('is-targeted')}

 applyLang(getLang());
 renderTheme();
 highlightRequestedDirection();
})();
