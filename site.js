...
(function(){
 function initArg(){
  ...
  branches.forEach(function(b,i){
   ...
   g.addEventListener('keydown',function(e){if(e.key==='Enter'||e.key===' '){e.preventDefault();select(i,true)}});
   btn.addEventListener('click',function(){select(i,true);root.querySelector('.arg-info').scrollIntoView({behavior:'smooth',block:'nearest'})}); 
  });
  ...
  function select(i, shouldScroll){
   ...
   if(shouldScroll && activeBtn)activeBtn.scrollIntoView({behavior:'smooth',block:'nearest',inline:'center'});
  }
