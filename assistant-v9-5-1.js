(function(){
'use strict';
if(window.__AROMIKA_ASSISTANT_V951__) return;
...
function romiHTML(kind,cls){
 ...
 return '<span class="aa-romi '+(cls||'')+'"><img src="'+esc(src)+'" data-romi-kind="'+esc(k)+'" data-romi-fallback-step="0" alt="" decoding="async" draggable="false"></span>'
}
function romiFile(kind){
 return kind==='launcher'?'romi-launcher.webp':kind==='finder'?'romi-finder.webp':kind==='success'?'romi-success.webp':'romi-greeting.webp'
}
function bindRomiImageFallback(){
 document.addEventListener('error',function(e){
  var img=e.target;
  if(!img||!img.matches||!img.matches('img[data-romi-kind]'))return;
  var kind=img.getAttribute('data-romi-kind')||'greeting';
  var step=parseInt(img.getAttribute('data-romi-fallback-step')||'0',10);
  var file=romiFile(kind);
  if(step===0){
   img.setAttribute('data-romi-fallback-step','1');
   var local=window.ROMI_LOCAL_ASSET_BASE||'';
   if(local){
    img.src=String(local).replace(/\/?$/,'/')+file+'?v=9.5.1';
    return;
   }
  }
  if(step<=1){
   img.setAttribute('data-romi-fallback-step','2');
   img.src='https://cdn.jsdelivr.net/gh/dickort/aromika-site@main/romi/'+file+'?v=9.5.1';
   return;
  }
...
function defaultUiState(){
 return {screen:'welcome',history:[],finderCategory:'',finderNeed:'',finderVolume:'',rating:0};
}
function hasChosenRomiLanguage(){
 try{return !!localStorage.getItem(ROMI_LANG_KEY)}catch(e){return false}
}
function loadUiState(){
 var s=defaultUiState(),saved=null;
 try{saved=JSON.parse(localStorage.getItem(UI_STATE_KEY)||'null')}catch(e){}
 if(saved&&typeof saved==='object'){
  ['screen','finderCategory','finderNeed','finderVolume','rating'].forEach(function(k){
   if(saved[k]!==undefined)s[k]=saved[k];
  });
  if(Array.isArray(saved.history))s.history=saved.history.slice(-8);
 }
 var allowed=['welcome','home','finder','finderNeeds','finderVolumes','finderResult','rep','partner','feedback','feedbackSuccess'];
 if(allowed.indexOf(s.screen)<0)s.screen='home';

 // Greeting is shown only until the user chooses a language once.
 if(hasChosenRomiLanguage()&&s.screen==='welcome')s.screen='home';
 if(!hasChosenRomiLanguage())s.screen='welcome';
...
function saveUiState(){
 try{
  localStorage.setItem(UI_STATE_KEY,JSON.stringify({
   ...
   finderCategory:state.finderCategory||'',
   finderNeed:state.finderNeed||'',
   finderVolume:state.finderVolume||'',
...
function gotoScreen(root,screen,data,push){if(push!==false)state.history.push(state.screen);state.screen=screen;if(data)Object.assign(state,data);saveUiState();render(root);scrollPanelTop(root)}
function back(root){var prev=state.history.pop()||'home';state.screen=prev;saveUiState();render(root);scrollPanelTop(root)}
function home(root){state.screen='home';state.history=[];saveUiState();render(root);scrollPanelTop(root)}
...
function bindScreen(root,t,l){
 ...
 var restart=root.querySelector('[data-aa-restart]');if(restart)restart.addEventListener('click',function(){state.history=[];saveUiState();gotoScreen(root,'finder',{finderCategory:'',finderNeed:'',finderVolume:''},false)});
 ...
 root.querySelectorAll('[data-aa-rating]').forEach(function(b){b.addEventListener('click',function(){state.rating=parseInt(b.getAttribute('data-aa-rating'),10)||0;saveUiState();root.querySelectorAll('.aa-star').forEach(function(s){s.classList.toggle('is-active',parseInt(s.getAttribute('data-aa-rating'),10)<=state.rating)});emit('feedback_rating',{rating:state.rating})})});
